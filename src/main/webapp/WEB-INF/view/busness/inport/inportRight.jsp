...
<form class="layui-form">
	...
	<script type="text/html"  id="inportToolBar">
...
layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    //新闻列表
     tableIns = table.render({
        elem: '#inportList',
        url : '${ctx }/inport/queryAllInport.action',
        cellMinWidth : 95,
        page : true,
        ...
        cols : [[
            ...
            {field: 'id', title: '进货ID', width:80, align:"center"},
            ...
            {title: '操作', width:130, templet:'#inportListBar' ,fixed:"right",align:"center"}
        ]]
    });
    //监听头工具栏事件
        ...
        case 'delete':
          if(data.length === 0){
            layer.msg('请选择一行');
              ...
              inportId = [];
              for (var i in data) {
                  inportId.push(data[i].id);
              }
              layer.confirm('确定删除选中的进货单？', {icon: 3, title: '提示信息'}, function (index) {
              	 //将需要删除的inportId作为json参数传入 
                  $.post("${ctx}/inport/batchDeleteInport.action",{inportIds : JSON.stringify(inportId) },function(data){
              	   layer.msg(data.msg);
                  tableIns.reload();
                  })
              })
    ...
    table.on('tool(inportList)', function(obj){
        ...
        if(layEvent === 'edit'){ //编辑
            ...
            layer.confirm('确定删除'+data.id+'编号'+data.goodsname+'商品的进货单？',{icon:3, title:'提示信息'},function(index){
                 $.post("${ctx}/inport/deleteInport.action",{
                	 id : data.id  //将需要删除的inportId作为参数传入
               },function(data){
            	   layer.msg(data.msg);
                    tableIns.reload();
                })
            ...
            layer.alert("此功能需要前台展示，实际开发中传入对应的必要参数进行进货内容页面访问")
        }
    });
})
</script>
